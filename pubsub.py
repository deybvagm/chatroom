...
class RabbitmqClient(object):
    ...
    def connect(self):
        ...
        LOGGER.info('[RabbitMqClient] Connecting to RabbitMQ on localhost, Object: %s ' % self)

        return TornadoConnection(parameters=self._parameters, on_open_callback=self.on_connection_opened)
    ...
    def on_connection_closed(self, connection, reply_text):
        ...
        LOGGER.warning('[RabbitMqClient] Rabbitmq connection closed unexpectedly : %s ' % connection)
        ...
        if self._closing:
            ...
            LOGGER.info("Connection closed, reopening in 5 seconds: reply_text : %s " % (
                reply_text))
    ...
    def open_channel(self):
        ...
        LOGGER.info('[RabbitMqClient] Creating a new channel for connection : %s ' % self._connection)
    ...
    def on_channel_closed(self, channel, reply_text):
        ...
        LOGGER.info("Channel %i was closed: reply_text : %s " % (channel, reply_text))
    ...
    def setup_exchange(self):
        ...
        LOGGER.info('[RabbitMqClient] Declaring exchange : %s ' % self.config.exchange)
        self._channel.exchange_declare(exchange=self.config.exchange,
    ...
    def on_queue_declareok(self, method_frame):
        ...
        self.bind_queue(self.config.binding_key)
    ...
    def bind_queue(self, binding_key):
        ...
        LOGGER.info('[RabbitMqClient] Binding %s to %s with %s ' %
                    (self.config.exchange, self._queue, binding_key))
    ...
    def enable_delivery_confirmations(self):
        ...
        LOGGER.info('[RabbitMqClient] Enabling delivery confirmation for publisher - Issuing Confirm.Select RPC command')
